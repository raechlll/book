...
# Inspect the data types of columns RawData, compName, expType and compConcentration 
CE_LIQ_FLOW_062_Tidydata %>% select(RawData, compName, compConcentration, expType)

# Change the columns compName, compConcentration and expType to numeric or factor
CE_LIQ_FLOW_062_Tidydata$expType   <- as.factor(as.character(CE_LIQ_FLOW_062_Tidydata$expType))
CE_LIQ_FLOW_062_Tidydata$compConcentration <- as.numeric(as.character(CE_LIQ_FLOW_062_Tidydata$compConcentration))
CE_LIQ_FLOW_062_Tidydata$compName <- as.factor(as.character(CE_LIQ_FLOW_062_Tidydata$compName))
...
```{r eval=T, message=F, fig.cap="Fig. 1: Scatterplot voor CE_LIQ_FLOW_062_Tidydata waarbij de concentratie van de verbinding op de x-as staat in log10, aantal nakomelingen op de y-as. Verschillende symbolen vertegenwoordigen verschillende experimenttypes."}
...
geom_jitter(alpha = 0.5, show.legend = TRUE, position = position_jitter(width = 0.1, seed = 0), size = 3) +
  theme_minimal() +
...
```{r eval=T, message=F, fig.cap= "Fig. 2:  Scatterplot van genormaliseerde data voor CE.LIQ.FLOW.062_Tidydata.xlsx waarbij de concentratie van de verbinding op de x-as staat, de genormaliseerde ruwe data tellingen op de y-as. Verschillende symbolen vertegenwoordigen verschillende experimenttypes."}
